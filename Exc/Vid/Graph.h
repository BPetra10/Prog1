...
struct Color {
	//egyértelmű azonosításra jó az enum, meghatározott érték
	enum Color_type {
	...
	//konstruktorok
	Color(Color_type cc): c(Fl_Color(cc)), v(visible) {}//látható szín
	Color(Color_type cc, Transparency vv): c(Fl_Color(cc)), v(vv) {}//láthatóság állításos
	Color(int cc): c(Fl_Color(cc)), v(visible) {}
	Color(Transparency vv): c(Fl_Color()), v(vv) {}

	int as_int() const { return c; } //getter függvény, szín visszaadásra
	char visibility() const { return v; } // láthatóság visszaadás
	void set_visibility(Transparency vv) { v = vv; }//láthatóság beállítás

private:
	unsigned char v; //visibility-re
	Fl_Color c;
};
...
struct Line_style {
	...
	//konnstruktorok
	Line_style(Line_style_type ss): s(ss), w(0) { } //stílus állítás, vastagság alapértelmezett
	Line_style(Line_style_type ss, int ww): s(ss), w(ww) { }//stílus és vastagság állítás px
...
struct Fill {
	//Konstruktor
	Fill() :no_fill(true), fcolor(0) { } //nincs kitöltve
	Fill(Color c) :no_fill(false), fcolor(c) { } //kitöltés
	//get, set:
...
struct Line : Shape {
	...
	//Konstruktor, ami várja a pontpárokat
	//A pontpárokat összekötjük a draw_line()-nal, ami a Shape függvénye
};

struct Lines : Shape {
	Lines(){}//alapértelmezett konstruktor
	Lines(initializer_list<Point> lst): Shape{lst} 
		{if ( lst.size() % 2 ) error("páratlan számú pontok");}
	/*pontok listájának inicializálós konstruktora
	A hibakezelés az, hogy mindig páros számú pontunk legyen
	hiszen vonalat úgy tudunk ábrázolni, hogy 2 pontot kötünk össze*/
	void draw_lines() const;//deklaráció
	void add(Point p1, Point p2) { Shape::add(p1); Shape::add(p2); }
	//A pontpár hozzáadás miatt kell egy add(), amin belül a Shape-addját használjuk
};

struct Open_polyline : Shape {
	//2 konstruktor: alap, listás
	Open_polyline(){};
