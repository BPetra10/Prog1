...
void Lines::draw_lines() const //jelezzük, hogy a Lines draw_lines()-át írjuk meg
{
	if (color().visibility())
	//fltk alap színe látható-e, azaz ha a visibility nem 0, kirajzolunk
		for (int i = 1; i < number_of_points(); i+=2)
			fl_line(point(i-1).x, point(i-1).y, point(i).x, point(i).y);
		//point() adott indexű pont
		//elkérjük a két összetartozó pont koordinátáját
}

void Open_polyline::draw_lines() const
{		//szín elkérés, és objektum látható-e
		if (fill_color().visibility()) {
			fl_color(fill_color().as_int());
			fl_begin_complex_polygon();//poly rajz kezdet
			for(int i=0; i<number_of_points(); ++i){
				fl_vertex(point(i).x, point(i).y); //élhúzás
			}
			fl_end_complex_polygon();
			fl_color(color().as_int());//alpaértelmezett szín
		}
		
		if (color().visibility())
			Shape::draw_lines();//hívjuk a kirajzoló függvényt
}
...
void Closed_polyline::draw_lines() const
{
	...
	//A végpontunkat és a kezdőpontunkat összekötjük
}
...
void Polygon::add(Point p)
{
	...
	//több a pontunk mint 1
	if (1<np) {
		//két pont ugyanaz-e
		if (p==point(np-1)) error("Same as last point");
		bool parallel; //egybeeső él vizsgálat
	...
	for (int i = 1; i<np-1; ++i) {
		...
		if (line_segment_intersect(point(np-1),p,point(i-1),point(i),ignore))
			error("crossing"); //keresztezik egymást
	}
...
void Polygon::draw_lines() const
{
		...
		Closed_polyline::draw_lines(); //3-nál kevesebb pont nem lehet
}
